...
request({
  ...
  _bodySprint = JSON.parse( body );
  _sprints    = _bodySprint.projects[0].lists ;
...
request({
  ...
  _status   = response.statusCode;
	...
	if ( _status === 200 )
	{
		...
		worksheet.cell( separador, 6 ).string( "List" ).style( style_cabecera );
		...
		for( var i = 0; i < _tasks.length ; i++ )
		{
			...
			listID   = _tasks[ i ].list;			
			for ( var k = 0 ; k < _sprints.length ; k++ )
			{
				if ( _sprints[ k ].id === _tasks[ i ].list.id )
				{
					listName = _sprints[ k ].name;
				}
			} 
			//console.log( _tasks[ i ].list );
			...
			if ( par === true )
			{
				...
				worksheet.cell( i + separador + 1, 6 ).string( validateNull( listName ) ).style( style );
			}
			else
			{
				...
				worksheet.cell( i + separador + 1, 6 ).string( validateNull(listName ) ).style( style_w );
			}
